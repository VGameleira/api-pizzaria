...
// Método HTTP: PUT
// Corpo da requisição (JSON): {id, nome, ingredientes, valor}
...
try {
    ...
    if (
        ...
        echo json_encode(array(
            "sucesso" => false,
            "mensagem" => "Dados incompletos. Forneça: id, nome, ingredientes, valor"
        ), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
        exit;
    }
    ...
    if ($pizza->update()) {
        ...
        echo json_encode(array(
            "sucesso" => false,
            "mensagem" => "Não foi possível atualizar a pizza"
        ), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
    }
} catch (Exception $e) {
    http_response_code(500);
