...
/**
 * API REST - Listar bebidas com paginação
 * 
 * Método HTTP: GET
...
try {
    ...
    $total = $bebida->count();
    $stmt  = $bebida->readPaginated($page, $limit);

    $dados = [];
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $dados[] = [
            'idBebida'  => (int) $row['idBebida'],
            'nome'      => $row['nome'],
            'alcoolica' => (int) $row['alcoolica'],
    ...
    Response::error(500, 'Erro ao buscar bebidas');
}
